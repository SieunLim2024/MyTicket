		...
		while (!quit) {
			...
			if (flag) {
				...
				if (str.toUpperCase().equals("Y")) {
					...
					// 잔여 좌석과 살 티켓 수량 비교
					if (Admin.performanceList.get(numId).getSoldSeats() + numTicket > Admin.performanceList.get(numId).getTotalSeats()) {
						System.out.println("잔여 좌석보다 구매 수량이 많습니다.");
					} else {
						// 수량 만큼 반복문 돌리기
						Performance.printSeats(performanceList, inputID);// 좌석 보여주기
						// 좌석 고르기
						String seatNum= askSeatNum(numTicket);//고른 좌석 받음.
						System.out.println("고른 좌석은 "+seatNum+" 입니다.");
						System.out.println(performanceList.get(numId).getName() + " 공연이 장바구니에 추가되었습니다.");

						// 장바구니에 넣기이미 카트에 항목 있으면 수량만 업데이트
						if (!cart.isCartInPerformance(performanceList.get(numId).getPerformanceID(), numTicket)) {	//이미 카트에 항목 있으면 수량만 업데이트
							cart.insertPerformance(performanceList.get(numId), numTicket, nowUser, seatNum);		//아니면 카트에 항목 추가
						}//end of if
	...
	private static void buy() {
		...
		for (int j = 0; j < paymentList.size(); j++) {
			for (int i = 0; i < Admin.performanceList.size(); i++) {
				if (Admin.performanceList.get(i).getName().equals(paymentList.get(j).getPerformanceName())) {
					Admin.performanceList.get(i).setSoldSeats(Admin.performanceList.get(i).getSoldSeats() + paymentList.get(j).getQuantity());
					//끊어주고 배열로....
					//여기 검정색으로 칠해줘야해!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				}
			}
